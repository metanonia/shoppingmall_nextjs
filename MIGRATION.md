...
- **네이버/다음 쇼핑 피드 설정 필드가 스키마에 있는데 아무도 안 읽음**:
  `Configuration.goods_engine_naver`/`goods_engine_daum`,
  `Goods.engine_use` — 소스 전체에서 참조 0건 확인(직접 grep 재검증 완료).
...
- 회원/주문 **엑셀 다운로드** 기능 — "배송송장 엑셀 **업로드**"만
  스코프아웃으로 문서화됐고 다운로드 계열은 언급 자체가 없었음.
