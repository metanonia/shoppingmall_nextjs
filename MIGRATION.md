...
우선순위 순 그룹(A~I)으로 나눠 구현 중이며, **그룹 A~I 대부분 완료**(기타정책·
관리자목록컬럼커스터마이징은 비용 대비 가치가 낮아 의도적 스코프컷, 회원/주문
엑셀 다운로드만 미착수로 남음) — 상세는 아래 "## 마이그레이션 완결성 감사" 섹션과
...
- **미착수(확인 필요)**: 회원/주문 **엑셀 다운로드** 기능 — "배송송장 엑셀
  **업로드**"만 스코프아웃으로 문서화됐고 다운로드 계열은 이번 감사에서도
  구현하지 못함. 다음 세션에서 이어서 처리할 것.
